            ...
            for index, video in enumerate(list_video_v):
                ...
                if taken_at:
                    try:
                        ...
                        print(
                            "[E] Could not determine timestamp filename for this file, using default: ") + final_filename
                else:
                    final_filename = filename.split('.')[0] + ".mp4"
                save_path_video = download_dest + "/stories/{}/".format(user_to_check) + final_filename.replace(".mp4",
                                                                                                                ".video.mp4")
                ...
                if not os.path.exists(save_path_final):
                    print(
                        "[I] ({:d}/{:d}) Downloading video: {:s}".format(index + 1, len(list_video_v), final_filename))
                    try:
